    ...
    QRCode.toDataURL(this.qrUrl, { errorCorrectionLevel: "H" }, (err, url) => {
      ...
      setTimeout(() => {
        ...
        if (this.qrLogo) {
          ...
          ctx.fillStyle = "#fff";
          ctx.beginPath();
           let logoSize=this.qrLogoSize?this.qrLogoSize:30;
          let logoPosition = (this.qrSize - logoSize) / 2; //logo相对于canvas居中定位
          let h = logoSize + 10; //圆角高 10为基数(logo四周白色背景为10/2)
          let w = logoSize + 10; //圆角宽
